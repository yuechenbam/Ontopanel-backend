...
class Individual(object):
    ...
    def _set_sameAs(self, term):
        ...
        if isinstance(term, (Individual, Identifier)):
            self.graph.add(
                (self.identifier, OWL.sameAs, classOrIdentifier(term)))
        else:
            for c in term:
                assert isinstance(c, (Individual, Identifier))
                self.graph.add(
                    (self.identifier, OWL.sameAs, classOrIdentifier(c)))
...
def DeepClassClear(classToPrune):
    ...
    classToPrune.graph.remove(
        (classToPrune.identifier, OWL.equivalentClass, None))
    inverseClass = classToPrune.complementOf
    if inverseClass:
        classToPrune.graph.remove(
            (classToPrune.identifier, OWL.complementOf, None))
...
class EnumeratedClass(OWLRDFListProxy, Class):
    ...
    def __init__(self, identifier=None, members=None, graph=None):
        ...
        rdfList = list(self.graph.objects(
            predicate=OWL.oneOf, subject=self.identifier))
...
class BooleanClass(OWLRDFListProxy, Class):
    ...
    def __init__(
        ...
        rdfList = list(self.graph.objects(
            predicate=operator, subject=self.identifier))
        assert (
...
class Restriction(Class):
    ...
    def serialize(self, graph):
        Property(self.onProperty, graph=self.graph,
                 baseType=None).serialize(graph)
        for s, p, o in self.graph.triples((self.identifier, None, None)):
            graph.add((s, p, o))
    ...
    def _set_someValuesFrom(self, other):
        triple = (self.identifier, OWL.someValuesFrom,
                  classOrIdentifier(other))
        if not other:
            return
        elif triple in self.graph:
    ...
    def _set_maxCardinality(self, other):
        triple = (self.identifier, OWL.maxCardinality,
                  classOrIdentifier(other))
        if not other:
            return
        elif triple in self.graph:
    ...
    def _set_minCardinality(self, other):
        triple = (self.identifier, OWL.minCardinality,
                  classOrIdentifier(other))
        if not other:
            return
        elif triple in self.graph:
...
some = Infix(
    lambda prop, _class: Restriction(
...
class Property(AnnotatableTerms):
    ...
    def setupVerbAnnotations(self, verbAnnotations):
        ...
        if TV_vbg:
            self.TV_vbgProp.extent = [
                (self.identifier, self.handleAnnotation(TV_vbg))]
    ...
    def __init__(
        ...
        if baseType is None:
            ...
            self._baseType = first(Individual(
                self.identifier, graph=self.graph).type)
        else:
    ...
    def _set_inverseOf(self, other):
        if not other:
            return
        self.graph.add((self.identifier, OWL.inverseOf,
                       classOrIdentifier(other)))
    ...
    def _set_domain(self, other):
        ...
        if isinstance(other, (Individual, Identifier)):
            self.graph.add((self.identifier, RDFS.domain,
                           classOrIdentifier(other)))
        else:
            for dom in other:
                self.graph.add(
    ...
    def _set_range(self, ranges):
        ...
        if isinstance(ranges, (Individual, Identifier)):
            self.graph.add((self.identifier, RDFS.range,
                           classOrIdentifier(ranges)))
        else:
            for range in ranges:
                self.graph.add((self.identifier, RDFS.range,
                               classOrIdentifier(range)))
